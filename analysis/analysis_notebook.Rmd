...
Location demonstrates strong collinearity with other variables. This isn't surprising given our analyses within years. Let's remove and look ant VIFs again.
...
```{r}
no_loc_1 <- glmmTMB(tlf ~ jday_c + geno_sex + density+sex_ratio_y_l + sex_ratio_rg_l + annual_n +geno_sex*jday_c + geno_sex*density + geno_sex*sex_ratio_y_l + geno_sex*sex_ratio_rg_l + (1|group) + (1|year) , data = mm_data, family = nbinom2)

no_loc_2 <- glmmTMB(tlf ~ jday_c + geno_sex + density + sex_ratio_rg_l + annual_n +geno_sex*jday_c + geno_sex*density  + geno_sex*sex_ratio_rg_l +annual_n + (1|group) + (1|year) , data = mm_data, family = nbinom2)

AIC(no_loc_1, no_loc_2)
```

Including annual sex ratios is a much better fit to the data than annual outplanting program size, but the moderate collineearity between these two variables should be considered. 

As a summary so far: location demonstrated collinearity with release date in some years. Any significant effect of release dateneeds to be considered in light of this confounding. We also considered two year-level fixed effects, total number of outplants in a year and the overall sex ratio among outplants in a year. These were only slightly collinear, but we should remember that any year level effects are confounded by all the other variables, considered here or otherwise, could be driving this pattern.
...
mm_beyond_opt <- glmmTMB(tlf ~ jday_c + geno_sex  +  density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l+ (1|group) + (1|year), data = mm_data, family = nbinom2)
mm_beyond_opt2 <- glmmTMB(tlf ~ jday_c + geno_sex +  density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l +(1|year), data = mm_data, family = nbinom2)
mm_beyond_opt3 <- glmmTMB(tlf ~ jday_c + geno_sex +  density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l +   (1|group), data = mm_data, family = nbinom2)
mm_beyond_opt4 <- glmmTMB(tlf ~ jday_c + geno_sex +  geno_sex* density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l, data = mm_data, family = nbinom2)
...
Wow this is interesting. Previously I fit an unsaturated model (without the new variables density and sex ratio and others) It found that the optimum random effects structure included year. Including sex ratios and densities as a fixed effect led to year as a random effect no longer providing much improvement to the model. This suggests that much of the interannual variation in TLF is actual due to interannual variation in density or sex ratios. Same thing goes for release group. After fitting release group level fixed effects, release group random effects captures a very small amount of variation and does not improve the model fit according to LRT, AIC or BIC. This also means that we don't need to fit a mixed model at all. 

Let's refit the model (using MASS instead of glmmTMB - now that we know we don't need it).

```{r}
beyond_opt <- glm.nb(tlf ~ jday_c + geno_sex  + geno_sex + density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l, data = mm_data)
summary(beyond_opt)
```
...
```{r}
beyond_opt_nl2 <- glm.nb(tlf ~ poly(jday_c,2) + geno_sex + location + geno_sex*location + poly(density,2)+ geno_sex*poly(density,2)+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + poly(density,2)*sex_ratio_y_l + poly(density,2)*sex_ratio_rg_l , data = mm_data)

beyond_opt_nld <- glm.nb(tlf ~ jday_c + geno_sex + location + geno_sex*location + poly(density,2)+ geno_sex*poly(density,2)+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + poly(density,2)*sex_ratio_y_l + poly(density,2)*sex_ratio_rg_l , data = mm_data)

beyond_opt_nlj <- glm.nb(tlf ~ poly(jday_c, 2) + geno_sex + location + geno_sex*location + density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l + density*sex_ratio_rg_l, data = mm_data)

AIC(beyond_opt_nlj, beyond_opt_nl2, beyond_opt_nld, beyond_opt)
BIC(beyond_opt_nlj, beyond_opt_nl2, beyond_opt_nld, beyond_opt)
```

Linear effects are best.

Let's use LRTs to evaluate if each effect significanty improves the model

```{r}
drop1(beyond_opt, test = "Chisq")
```
...
```{r}
drop1(glm.nb(tlf ~ jday_c + geno_sex  + geno_sex + density+ geno_sex*density+sex_ratio_y_l + sex_ratio_rg_l + geno_sex*sex_ratio_rg_l + geno_sex*sex_ratio_y_l + density*sex_ratio_y_l , data = mm_data), test = "Chisq")
```
...
```{r}
final_model <- glm.nb(tlf ~ jday_c + geno_sex  +sex_ratio_y_l ,  data = mm_data)
plot(final_model)
qqrplot(final_model)
...
Males have lower fitness than females. 
...
```{r}
pred1 <- as.data.frame( cbind(exp(mm_data$sex_ratio_y_l),predict.glm(final_model, type = "response", terms = "sex_ratio_y_l")))
...
```{r}
day_effect <- emmeans(final_model, specs = pairwise ~ jday_c, type = "response" )
day_effect

sex_ratio_effect <- emmeans(final_model, specs = pairwise ~ sex_ratio_y_l, type = "response" )
sex_ratio_effect

```

```{r}
emmeans(final_model, specs = "geno_sex", by="sex_ratio_y_l", type = "response")

emmeans(final_model, specs = "geno_sex", by="jday_c", type = "response")
