...
above_glm <- glm.nb(tlf ~  type, data = filter(parents, year == 2015, type %in% c("outplant", "reintro_above", "reintro")) )
summary(above_glm)
rootogram(above_glm)
...
#eff1 <- predictorEffect("sex_ratio_y_l", final_model, focal.levels = seq(-0.4,0.7,by = 0.1))e
eff1 <- predictorEffect("sex_ratio_rg_l", M2011_NB)
...
actual_means <- mm_data %>%
  group_by(year, geno_sex, sex_ratio_rg_l) %>%
  summarise(mean_tlf = mean(tlf))

ggplot(data = effdf, aes(x = (sex_ratio_rg_l), y = fit, color = geno_sex))+ 
  geom_line()
...
ggplot(data = effdf, aes(x = (sex_ratio), y = fit, color = geno_sex))+ 
  ...
  geom_rug(data = mm_data, aes(x = sex_ratio_rg_l, y = NULL, color = NULL)) + 
  geom_point(data = actual_means, aes(x = sex_ratio_rg_l, y = mean_tlf, color = geno_sex), alpha = 0.8, shape = 24, size = 2)
...
```{r}
require(emmeans)
day_effect <- emmeans(glm2013_final, type = "response" )
day_effect
plot(day_effect)+theme_classic()+xlab("Estimated Marginal Mean TLF\nwith 95% CIs")+ylab("day")
```
...
ggplot(data = eff_sr_df, aes(x = (sex_ratio), y = fit))+ 
  ...
  geom_point(data = actual_means, aes(x = sex_ratio_rg, y = mean_tlf), alpha = 0.8, shape = 24, size = 2)+ggtitle("2013 Predictors of Fitness - Release Group Sex Ratio")
...
ggplot(data = eff_d_df, aes(x = (density), y = fit))+ 
  ...
  ggtitle("2013 Predictors of Fitness - Density")
...
ggplot(data = eff_j_df, aes(x = (jday), y = fit))+ 
  ...
  ggtitle("2013 Predictors of Fitness - Release Day")
```
...
ggplot(data = pe_j_2015, aes(x = (jday), y = fit))+ 
  ...
  geom_point(data = actual_means, aes(x = jday, y = mean_tlf), alpha = 0.8, shape = 24, size = 2)+ggtitle("2015 Predictors of Fitness - Release Day")
...
ggplot(data = effdf, aes(x = (density), y = fit, color = geno_sex))+ 
  ...
  xlab("Density") +
  geom_smooth( aes(ymin = lower, ymax = upper, fill = geno_sex, colour = geno_sex), stat = "identity") +
  theme_bw()+ylab("TLF")+coord_cartesian(ylim = c(0, 2)) + scale_color_manual(labels = c("Female", "Male"), name = "Sex", values = c("#228833", "#AA3377")) + scale_fill_manual(labels = c("Female", "Male"), name = "Sex", values = c("#228833", "#AA3377")) +
...
actual_means <- mm_data %>%
  group_by( geno_sex, sex_ratio_rg) %>%
  summarise(mean_tlf = mean(tlf))
